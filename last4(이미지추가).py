...
def SearchButtonAction():
    ...
    global photo
    # 해당 파일이 존재하는지 확인을 한다.
    if ( os.path.isfile("ImageFile\\" + InputLabel.get() + ".gif") == True):
        # 파일이 존재할 경우 해당 gif 파일을 불러온다.
        photo = PhotoImage(file="ImageFile\\" + InputLabel.get() + ".gif")
        imageLabel = Label(g_Tk, image=photo)
        imageLabel.pack()
        imageLabel.place(x=700, y=180)
    else:
        # 파일이 존재하지 않을 경우 NoImage를 불러온다.
        photo = PhotoImage(file="ImageFile\\NoImage.gif")
        imageLabel = Label(g_Tk, image=photo)
        imageLabel.pack()
        imageLabel.place(x=700, y=180)
    for name in AllmenuList:
        number = number + 1
        # number < len(AllmenuList)를 하는 이유는 범위를 초과해서 RenderList를 읽을 수 있기 때문에, AllmenuList 범위보다 아래일 경우에만 추가를 한다.
        if(name.find(InputLabel.get()) != -1 and number < len(AllmenuList)):
